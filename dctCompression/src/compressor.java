...
public class compressor{ //use dct to compress
	...
	public compressor(String infile, String outfile, int flag){
		...
		try{
			...
			if((width%8)!=0){if((height%8)!=0){throw new Exception("Image pixels not divisible by 8!");}}
			...
			for(int a=0;a<(width/8);a++){
				for(int b=0;b<(height/8);b++){
					for(int i=0;i<8;i++){
						for(int j=0;j<8;j++){
							imageBlocks[a+b][i][j][0]=pixelColor[(a*8)+i][(b*8)+j][0];
							imageBlocks[a+b][i][j][1]=pixelColor[(a*8)+i][(b*8)+j][1];
							imageBlocks[a+b][i][j][2]=pixelColor[(a*8)+i][(b*8)+j][2];
						}
					}
				}
